...
const { extractSlug, artistRegex } = require("./utils/extractSlug")
...
const extractNovelTypeListData = ($) => {
    return $("div.wpb_wrapper > ul > li").map((i, el) => {
        ...
        return {
            ...
            slug: extractSlug(element.find("a").attr("href"), artistRegex),
...
const requestData$ = async (requestUrl) => {
    ...
    const url = `https://www.novelupdates.com/list-artists/?pg=${params.page}`
